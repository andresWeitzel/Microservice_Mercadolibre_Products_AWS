//Services
import { getAllProductSpecifService } from "src/services/productSpecification/getAllProductSpecifService";
//Enums
...
module.exports.handler = async (event: any) => {
  try {
    ...
    //-- start with db query  ---
    productList = await getAllProductSpecifService(pageSizeNro, pageNro, orderBy);
    //-- end with db query  ---

    return productList;
  } catch (error) {
    msg = `Error in GET ALL PRODUCT SPECIFICATION CONTROLLER lambda. Caused by ${error}`;
